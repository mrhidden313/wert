<script>
	...
	let impersonatingId = $state(null);

	async function handleImpersonate(accountId) {
		impersonatingId = accountId;
		try {
			...
			alert('Network or server error during impersonation.');
		} finally {
			...
			<tbody class="divide-y divide-gray-800/60 font-sans">
				{#each filteredAccounts as account}
					...
					>
						...
						<!-- Action Buttons -->
						<td class="px-6 py-4 whitespace-nowrap text-right text-xs" onclick={(e) => e.stopPropagation()}>
							<div class="flex items-center justify-end gap-2">
								<button
									type="button"
									onclick={() => handleImpersonate(account.id)}
									disabled={impersonatingId === account.id}
									class="px-2.5 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:border-emerald-500/50 rounded-lg font-bold transition-all inline-flex items-center gap-1.5 shadow-sm active:scale-95 disabled:opacity-50"
									title="Open Live Workspace (Ghost Login as Client)"
								>
									{#if impersonatingId === account.id}
								...
								<a
									...
									class="px-2.5 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-200 hover:text-white rounded-lg font-semibold transition-colors inline-flex items-center gap-1"
								>
									<span>Details</span>
			...
			<div class="flex items-center justify-between gap-2 pt-1">
				...
				<div class="flex items-center gap-2">
					...
					<button
						...
						onclick={() => handleImpersonate(account.id)}
						disabled={impersonatingId === account.id}
						class="px-2.5 py-1.5 bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/40 rounded-lg text-xs font-extrabold transition-all flex items-center gap-1 active:scale-95 disabled:opacity-50"
						title="Open Live Workspace (Ghost Login)"
					>
						{#if impersonatingId === account.id}
							<span class="w-3 h-3 border-2 border-emerald-400 border-t-transparent rounded-full animate-spin"></span>
						{:else}
