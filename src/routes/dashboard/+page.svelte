...
<!-- Header -->
<div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
	<div>
		<h1 class="text-xl sm:text-2xl font-bold text-white">All Accounts</h1>
		<p class="text-gray-400 mt-1 text-sm">Manage workspaces, subscriptions, and access.</p>
	</div>
	<div class="flex items-center gap-2 flex-wrap">
		<a href="/dashboard/god-mode" class="px-3 py-2 bg-gradient-to-r from-red-600 to-purple-600 hover:from-red-500 hover:to-purple-500 text-white text-xs font-bold rounded-lg shadow-[0_0_15px_rgba(220,38,38,0.5)] transition-all flex items-center gap-2">
			<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
			GOD MODE
		</a>
		<a href="/dashboard/create-account" class="px-3 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-medium rounded-lg transition-colors whitespace-nowrap">
			+ Build Account
		</a>
	</div>
</div>
...
<div class="mb-6 border-b border-gray-800">
	<div class="flex items-center justify-between">
		<nav class="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto no-scrollbar" aria-label="Tabs">
			<button 
				class="{currentTab === 'active' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-400 hover:text-gray-300 hover:border-gray-700'} whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors"
				onclick={() => currentTab = 'active'}>
				Active
			</button>
			<button 
				class="{currentTab === 'expired' ? 'border-yellow-500 text-yellow-400' : 'border-transparent text-gray-400 hover:text-gray-300 hover:border-gray-700'} whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors"
				onclick={() => currentTab = 'expired'}>
				Expired
			</button>
			<button 
				class="{currentTab === 'suspended' ? 'border-red-500 text-red-400' : 'border-transparent text-gray-400 hover:text-gray-300 hover:border-gray-700'} whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors"
	...
	<div class="flex flex-wrap items-center gap-2 py-3">
		<div class="flex items-center gap-1.5">
			...
			<select bind:value={sortBy} class="bg-gray-900 border border-gray-700 text-gray-300 text-xs rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block p-1.5 cursor-pointer">
				...
				<option value="days_high">Days (High→Low)</option>
				<option value="days_low">Days (Low→High)</option>
			</select>
		</div>
		<div class="flex items-center gap-1.5">
...
<!-- Desktop Table -->
<div class="hidden sm:block bg-gray-900 border border-gray-800 rounded-xl shadow-sm overflow-hidden">
				...
				{#each filteredAccounts as account}
					...
					>
						...
						<td class="px-6 py-4 whitespace-nowrap flex flex-col gap-1 items-start" onclick={(e) => e.stopPropagation()}>
							...
							{:else}
								<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-900/30 text-red-400 border border-red-500/20">Suspended</span>
							{/if}
							{#if account.freeze}
								<form method="POST" action="?/toggleFreeze" use:enhance class="m-0 mt-1">
									<input type="hidden" name="accountId" value={account.id} />
									<input type="hidden" name="freeze" value="false" />
									<button type="submit" title="Click to Unfreeze App" class="px-2.5 py-0.5 inline-flex items-center gap-1.5 text-xs leading-5 font-semibold rounded-full bg-orange-900/40 hover:bg-orange-900/80 text-orange-400 border border-orange-500/30 transition-all cursor-pointer shadow-sm">
...
<div class="sm:hidden space-y-3">
	{#each filteredAccounts as account}
		<div 
			class="bg-gray-900 border border-gray-800 rounded-xl p-4 cursor-pointer hover:bg-gray-800/50 transition-colors"
			style="border-left: 4px solid {account.labelColor || 'gray'};"
			onclick={() => goto(`/dashboard/account/${account.id}`)}
		>
			<div class="flex items-start justify-between mb-3">
				<div class="min-w-0 mr-2">
					<div class="font-semibold text-emerald-400 text-sm truncate">{account.name}</div>
					<div class="text-xs text-gray-500">ID: {account.id}</div>
					<div class="text-xs text-gray-400 mt-0.5 truncate">{account.linkedEmail}</div>
				</div>
				<div class="flex flex-col items-end gap-1 shrink-0">
					{#if account.status === 'active'}
						<span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-blue-900/30 text-blue-400 border border-blue-500/20">Active</span>
					{:else}
						<span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-red-900/30 text-red-400 border border-red-500/20">Suspended</span>
					{/if}
					{#if account.freeze}
						<form method="POST" action="?/toggleFreeze" use:enhance class="m-0" onclick={(e) => e.stopPropagation()}>
							<input type="hidden" name="accountId" value={account.id} />
							<input type="hidden" name="freeze" value="false" />
							<button type="submit" class="px-2 py-0.5 text-xs font-semibold rounded-full bg-orange-900/40 text-orange-400 border border-orange-500/30">Frozen ×</button>
						</form>
					{/if}
				</div>
			</div>
			<div class="flex items-center justify-between" onclick={(e) => e.stopPropagation()}>
				<form method="POST" action="?/updatePlan" use:enhance class="m-0">
					<input type="hidden" name="accountId" value={account.id} />
					<select name="planType" onchange={(e) => e.target.form.requestSubmit()} class="bg-emerald-900/30 border border-emerald-500/30 text-emerald-400 text-xs font-semibold rounded-lg px-2 py-1 cursor-pointer">
						<option value="Trial" selected={account.planType === 'Trial'}>Trial</option>
						<option value="Monthly" selected={account.planType === 'Monthly' || account.planType === 'Unknown'}>Monthly</option>
						<option value="Yearly" selected={account.planType === 'Yearly'}>Yearly</option>
					</select>
