...
<div class="mb-6 flex items-center space-x-4">
	...
	<div>
		<h1 class="text-xl sm:text-2xl font-bold text-white">Billing & Pricing Dashboard</h1>
		<p class="text-gray-400 mt-1 text-sm">Financial analytics and pending dues overview.</p>
	</div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
	...
	<div class="flex flex-row lg:flex-col gap-4">
		<div class="bg-gradient-to-br from-emerald-900/40 to-gray-900 border border-emerald-500/20 p-4 sm:p-6 rounded-xl shadow-sm flex-1 flex flex-col justify-center">
			<h3 class="text-emerald-500 text-xs font-medium uppercase tracking-wider mb-1 sm:mb-2">Total Revenue</h3>
			<p class="text-2xl sm:text-4xl font-bold text-white">Rs {data.analytics?.totalRevenue?.toLocaleString() || 0}</p>
		</div>
		<div class="bg-gradient-to-br from-orange-900/40 to-gray-900 border border-orange-500/20 p-4 sm:p-6 rounded-xl shadow-sm flex-1 flex flex-col justify-center">
			<h3 class="text-orange-500 text-xs font-medium uppercase tracking-wider mb-1 sm:mb-2">Pending Dues</h3>
			<p class="text-2xl sm:text-4xl font-bold text-white">Rs {data.analytics?.totalPending?.toLocaleString() || 0}</p>
		</div>
	</div>

	<!-- Paid vs Pending Chart -->
	<div class="bg-gray-900 border border-gray-800 p-4 sm:p-6 rounded-xl shadow-sm">
	...
	<div class="overflow-x-auto">
		<table class="min-w-full divide-y divide-gray-800">
			<thead class="bg-gray-950/50">
				<tr>
					<th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Account</th>
					<th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden sm:table-cell">Startup Fee</th>
					<th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider hidden md:table-cell">Monthly Fees</th>
					<th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-emerald-400 uppercase tracking-wider">Total Due</th>
					<th scope="col" class="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-400 uppercase tracking-wider">Action</th>
				</tr>
			</thead>
				...
				{#each data.billingAccounts || [] as acc}
					<tr class="hover:bg-gray-800/50 transition-colors">
						<td class="px-4 sm:px-6 py-4 whitespace-nowrap">
							...
							<div class="text-xs text-gray-500 truncate max-w-[120px] sm:max-w-none">{acc.linkedEmail}</div>
						</td>
						...
						<td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right">
							<a href="/dashboard/account/{acc.id}" class="px-3 py-1.5 sm:px-4 sm:py-2 bg-blue-600/20 text-blue-500 hover:bg-blue-600/30 rounded-lg text-xs sm:text-sm font-medium transition-colors">
								Open Ledger
							</a>
						</td>
					</tr>
