	...
	<div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-lg">
		<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
			<div class="flex items-start gap-4">
				...
				<div>
					...
					<p class="text-xs text-gray-400 mt-1.5 max-w-xl leading-relaxed">
						When toggled ON, triggers the vibrant uncloseable System Upgrade & Maintenance blocker screen across all active client apps. Users will not see any close (X) or cancel button until you disable it here.
					</p>
				</div>
			</div>
			...
			<button
				...
				disabled={isSaving}
				class="px-6 py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2.5 {isGlobalMaintenance ? 'bg-red-600 hover:bg-red-700 text-white shadow-[0_0_25px_rgba(220,38,38,0.5)]' : 'bg-emerald-600 hover:bg-emerald-500 text-white'}"
			>
				{isGlobalMaintenance ? 'Disable Global Maintenance' : 'Enable Global Maintenance'}
			</button>
		</div>
	</div>
</div>
